...
class PostManager(models.Manager):
    def valid_posts(self, u=None, **kwargs):
        ...
        query = query.filter(
            models.Q(spam=Post.NOT_SPAM)
            | models.Q(spam=Post.DEFAULT)
            | models.Q(root__spam=Post.NOT_SPAM)
            | models.Q(root__spam=Post.DEFAULT)
        )
...
class AwardManager(models.Manager):
    def valid_awards(self):
        ...
        query = query.filter(
            models.Q(post__status=Post.OPEN) | models.Q(post__root__status=Post.OPEN)
        )
...
class Post(models.Model):
    ...
    lastedit_user = models.ForeignKey(
        User, related_name="editor", null=True, on_delete=models.CASCADE
    )

    # The user that last contributed to the thread.
    last_contributor = models.ForeignKey(
        User, related_name="contributor", null=True, on_delete=models.CASCADE
    )
    ...
    def title_prefix(self):
        ...
        if self.is_spam:
            ...
            prefix = (
                ...
                if self.is_open
                else f"{self.get_status_display()}:"
            )
    ...
    def high_spam_score(self, threshold=None):
        ...
        return (
            (self.spam_score > threshold) or self.is_spam or self.author.profile.low_rep
        )
    ...
    def update_parent_counts(self):
        ...
        descendants = Post.objects.filter(root=self.root).exclude(
            Q(pk=self.root.pk) | Q(status=Post.DELETED) | Q(spam=Post.SPAM)
        )
        ...
        Post.objects.filter(pk=self.parent.pk, is_toplevel=False).update(
            comment_count=com_count, answer_count=0, reply_count=children.count()
        )
...
class Subscription(models.Model):
    ...
    SUB_CHOICES = [
        ...
        (NO_MESSAGES, "Not subscribed"),
    ]
    TYPE_MAP = {
        Profile.NO_MESSAGES: NO_MESSAGES,
    ...
    def save(self, *args, **kwargs):
        ...
        if self.type is None:
            self.type = self.TYPE_MAP.get(
                self.user.profile.message_prefs, self.NO_MESSAGES
            )
