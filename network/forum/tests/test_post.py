...
from network.forum import models, views, search, tasks
...
class PostTest(TestCase):
    def setUp(self):
        ...
        self.owner = User.objects.create(
            username=f"test", email="tested@tested.com", password="tested"
        )
        self.staff_user = User.objects.create(
            username=f"test2",
            is_superuser=True,
            is_staff=True,
            email="tested@staff.com",
            password="tested",
        )
    ...
    def test_populate(self):
        ...
        management.call_command(
            "populate", n_users=10, n_messages=10, n_votes=10, n_posts=10
        )
    ...
    def process_response(self, response):
        ...
        self.assertEqual(
            response.status_code, 302, f"Could not redirect :\nresponse:{response}"
        )
