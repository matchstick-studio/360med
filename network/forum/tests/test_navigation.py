...
from network.forum import auth, models
...
class ForumNavigation(TestCase):
    def setUp(self):
        ...
        self.owner = User.objects.create(
            username=f"tested{get_uuid(10)}", email="tested@tested.com"
        )
    ...
    def visit_urls(self, urls, codes):
        c = Client()
        c.login(
            username=self.owner.username, email="tested@tested.com", password="tested"
        )
        for url in urls:
            print(url)
            ...
            if code not in codes:
                ...
                logger.error(f"")
                logger.error(
                    f"Error accessing: {url}, code={code} not in expected values {codes}"
                )
