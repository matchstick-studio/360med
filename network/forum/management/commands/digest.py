...
from network.accounts import util, models
...
def send_digests(days=1, subject=""):
    ...
    send_email(
        template_name=email_template, extra_context=context, recipient_list=emails
    )
...
def send_watched_tags():
    ...
    tags = Tag.objects.filter(
        post__lastedit_date__gt=delta, post__type__in=Post.TOP_LEVEL
    )
...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        ...
        parser.add_argument(
            "--daily", dest="daily", action="store_true", help="Send daily digests."
        )
        parser.add_argument(
            "--weekly", dest="weekly", action="store_true", help="Send weekly digests."
        )
        parser.add_argument(
            "--monthly",
            dest="monthly",
            action="store_true",
            help="Send monthly digests.",
        )
        parser.add_argument(
            "--tags",
            dest="tags",
            action="store_true",
            help="Send watched tags digests to users.",
        )
