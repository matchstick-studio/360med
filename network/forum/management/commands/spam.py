...
from network.forum import search, spam
...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        ...
        parser.add_argument(
            ...
            action="store_true",
            default=False,
            help="Resets the indexed flags.",
        )
        parser.add_argument(
            "--remove",
            action="store_true",
            default=False,
            help="Removes the existing index.",
        )
        parser.add_argument(
            "--test",
            action="store_true",
        ...
        parser.add_argument(
            "--niter", type=int, default=100, help="Number of iterations to test."
        )
        parser.add_argument(
            "--nsize", type=int, default=1500, help="Size of index per iteration."
        )
        parser.add_argument(
            "--limitmb",
            type=int,
            default=1024,
            help="Limit the size of the index buffer when testing",
        )
        parser.add_argument(
            "--index",
            action="store_true",
            default=False,
            help="How many posts to index",
        )
    ...
    def handle(self, *args, **options):
        ...
        if test:
            spam.test_classify(
                niter=niter, size=nsize, limitmb=limitmb, verbosity=verbosity
            )
