...
def send_award_message(sender, instance, created, **kwargs):
    ...
    if created:
        ...
        # Send local message
        tasks.create_messages(
            template=template, extra_context=context, rec_list=[instance.user]
        )
...
def ban_user(sender, instance, created, **kwargs):
    ...
    if instance.state == Profile.BANNED:
        ...
        Message.objects.filter(
            Q(sender=instance.user) | Q(recipient=instance.user)
        ).delete()
...
def finalize_post(sender, instance, created, **kwargs):
    ...
    subs = Subscription.objects.filter(
        date__gte=instance.lastedit_date, post=instance.root
    )
    ...
    tasks.notify_followers.spool(
        subs=subs, author=instance.author, extra_context=extra_context
    )
