"""
Markdown parser to render the network style markdown.
"""
import re
import inspect
...
from network.forum import auth
from network.forum.models import Post, Subscription
from network.accounts.models import Profile, User

# Test input.
TEST_INPUT = '''

https://www.networks.org/p/1 https://www.networks.org/p/2
...
http://test.networks.org/accounts/profile/user-2/ 

http://test.networks.org/p/p371285/
...
class networkInlineGrammer(InlineGrammar):
    """
...
class networkInlineLexer(MonkeyPatch):
    grammar_class = networkInlineGrammer
    ...
    def __init__(self, root=None, allow_rewrite=False, *args, **kwargs):
        ...
        super(networkInlineLexer, self).__init__(*args, **kwargs)
        self.enable_all()
...
def parse(text, post=None, clean=True, escape=True, allow_rewrite=False):
    ...
    renderer = networkRenderer(escape=escape)

    # Initialize the lexer
    inline = networkInlineLexer(renderer=renderer, root=root, allow_rewrite=allow_rewrite)
