...
class EditProfile(forms.Form):
    ...
    gender = forms.ChoiceField(required=True, label="Gender", choices=Profile.GENDER_CHOICES,
                                widget=forms.Select(attrs={'class': "ui dropdown"}),
                                help_text="""Choose your gender""")

    occupation = forms.ChoiceField(required=True, label="Occupation", choices=Profile.OCCUPATION_CHOICES,
                                widget=forms.Select(attrs={'class': "ui dropdown"}),
                                help_text="""What do you do?""")

    qualifications = forms.CharField(label="Qualifications", max_length=100, required=False,
                                help_text="""e.g MBChB, M MED OBS & GYN""")

    expertise = forms.CharField(label="Expertise", max_length=500, required=False,
                              help_text="""
                              Your specific areas of expertise or interest
                              """, widget=forms.HiddenInput())

    phone = forms.CharField(required=False, initial="+256", widget=PhoneNumberPrefixWidget(attrs={'placeholder': "Mobile number"}), 
                            label="Phone Number", help_text="If you would like to be reached by phone")
