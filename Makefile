...
DJANGO_SETTINGS_MODULE := network.server.settings
...
.PHONY:  forum accounts demo

# database to copy
export COPY_DATABASE := forum.db
...
accounts:
	...
	$(eval DJANGO_SETTINGS_MODULE := network.accounts.settings)
	$(eval DJANGO_APP := network.accounts)

emailer:
	@echo "*** Setting variables for emails app."
	$(eval DJANGO_SETTINGS_MODULE := network.emailer.settings)
	$(eval DJANGO_APP := network.emailer)

forum:
	@echo "*** Setting variables for forum app."
	$(eval DJANGO_SETTINGS_MODULE := network.forum.settings)
	$(eval DJANGO_APP := network.forum)
...
test:
	@echo DJANGO_SETTINGS_MODULE=${DJANGO_SETTINGS_MODULE}
	@echo DJANGO_APP=${DJANGO_APP}
	coverage run manage.py test ${DJANGO_APP} --settings network.server.test_settings -v 2 --failfast
...
# Temporary command to deploy forum on test server - with directory name being ~/network-engine/
